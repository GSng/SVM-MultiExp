...
if ncol_train_sp_case<n_col
    train_sp_case = [train_sp_case zeros(size(train_sp_case,1),(n_col-ncol_train_sp_case))];
end
if ncol_claim_mat<n_col
    claim_mat = [claim_mat zeros(size(claim_mat,1),(n_col-ncol_claim_mat))];
end
if ncol_test_sp_case<n_col
    test_sp_case = [test_sp_case zeros(size(test_sp_case,1),(n_col-ncol_test_sp_case))];
end

train_claim_ids = [train_sp_case(:,1); claim_mat(:,1)];
test_claim_ids = [test_sp_case(:,1); claim_mat(:,1)];
...
n_claim = size(train_claim_id_time_mat,1);
split_created = 0;

while ~isempty(start_loc) && start_loc<n_claim && split_created <max_split
    train_start_date = train_claim_id_time_mat(start_loc,2);    
